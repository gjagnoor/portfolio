...
const Profile = () => { 
    const data = useStaticQuery(
        graphql`
        query {
            allMarkdownRemark(sort: {order: ASC, fields: frontmatter___position}, filter: {frontmatter: {type: {eq: "workexperience"}}}) {
              edges {
                node {
                  frontmatter {
                    period
    ...
    useEffect(() => {
        setSteps(data.allMarkdownRemark.edges.map(({ node }) => node.frontmatter.company))
    }, [])
    ...
    function getContent(step) {
        return data.allMarkdownRemark.edges[step].node.html ? <div dangerouslySetInnerHTML={{ __html: data.allMarkdownRemark.edges[step].node.html }} className={`workexperiencecontent`}></div> : ""
    }
  return (
                              ...
                              <Step key={index}>
                                  ...
                                  >
                                      ...
                                      <StepLabel StepIconComponent={QontoStepIcon}>
                                          <span style={{ marginTop: "1.4em", fontFamily: 'Share Tech Mono, monospace', color: (activeStep === index ? "#04d9ff" : "gray"), fontSize: `1.2em`, whiteSpace: "nowrap" }}>
                                              <span style={{ color: "#04d9ff", fontFamily: "Share Tech Mono, monospace" }}>
                                                  0{index}.
                                              </span>
                                              {label}
                                          </span>
